...
return {
	...
	config = function()
		...
		vim.keymap.set("n", "<leader>fsg", function() builtin.grep_string({ search = vim.fn.input("Grep > ") }) end, { desc = "[f]ind [s]tring" })
		vim.keymap.set("n", "<leader>fw", function() local word = vim.fn.expand("<cword>") builtin.grep_string({ search = word }) end, { desc = "[f]ind [w]ord" })
		vim.keymap.set("n", "<leader>fW", function() local word = vim.fn.expand("<cWORD>") builtin.grep_string({ search = word }) end, { desc = "[f]ind [W]ORD" })
	end,
}
